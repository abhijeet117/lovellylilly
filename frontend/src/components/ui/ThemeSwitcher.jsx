...
const ThemeSwitcher = ({ id, className = '' }) => {
  useEffect(() => {
    ...
    document.querySelectorAll('.sw').forEach(s => {
      const onClick = () => setTheme(s.dataset.t)
      s.addEventListener('click', onClick)
      swListeners.push({ s, onClick })
    })

    return () => {
      swListeners.forEach(({ s, onClick }) => s.removeEventListener('click', onClick))
    }
  }, [])

  return (
    <div id={id} className={`swatches ${className}`}>
      <button className="sw on" data-t="dark" style={{ background: '#1a1208' }} title="Dark" aria-label="Switch to Dark theme" />
      <button className="sw" data-t="light" style={{ background: '#f0ece4' }} title="Light" aria-label="Switch to Light theme" />
      <button className="sw" data-t="sand" style={{ background: '#ddd0b0' }} title="Sand" aria-label="Switch to Sand theme" />
      <button className="sw" data-t="ocean" style={{ background: '#071828' }} title="Ocean" aria-label="Switch to Ocean theme" />
    </div>
  );
};
